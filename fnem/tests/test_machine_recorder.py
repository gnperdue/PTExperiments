...
class TestMachineRecorder(unittest.TestCase):
    ...
    def test_writeread_data(self):
        ...
        a0 = np.concatenate(([t0], meas0, targ0))
        success = self.recorder.write_data(t0, meas0, targ0)
        ...
        a1 = np.concatenate(([t1], meas1, targ1))
        success = self.recorder.write_data(t1, meas1, targ1)
        self.assertTrue(success)
