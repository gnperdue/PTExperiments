...
class FashionMNISTDataset(Dataset):
    ...
    def __init__(self, hdf5_file, transform=None, flatten=False):
...
class Standardize(object):
    ...
    def __init__(self, mean_file, std_file, flatten=False):
    ...
    def __call__(self, sample):
        if self._flatten:
            img = (sample['image'] - self.mean.reshape(1, 784)) / \
                self.std.reshape(1, 784)
        else:
...
def make_data_loaders(
    data_dir, batch_size, flatten=False
):
    testfile, trainfile, meanfile, stdfile = make_file_paths(data_dir)

    try:
        standardizer = Standardize(
            mean_file=meanfile, std_file=stdfile, flatten=flatten
        )
    ...
    fashion_trainset = FashionMNISTDataset(trainfile, trnsfrms, flatten)
    fashion_testset = FashionMNISTDataset(testfile, trnsfrms, flatten)
